...
execute if score #ret mh.temp matches 0 run tellraw @s "\u00a7c名称含有非法字符"


## 调用从书的内容读取id尝试选择的函数
data remove storage gu:main out
execute unless score #ret mh.temp matches 0 run function mh:compass/select/writedown with entity @s Inventory[{Slot:-106b}].components."minecraft:writable_book_content".pages[0]

# early return 
execute unless data storage gu:main out run item modify entity @s weapon.offhand {function:"set_components",components:{"minecraft:writable_book_content":{pages:[]}}}
