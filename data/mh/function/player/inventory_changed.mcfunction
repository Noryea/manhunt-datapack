...
execute if entity @s[team=hunters] unless score 猎人可追踪:敌人 mh.settings matches 1 unless score 猎人可追踪:队友 mh.settings matches 1 run scoreboard players set #can_have_compass mh.temp 0
execute if entity @s[team=runners] store result score #runners_count mh.temp if entity @a[team=runners,gamemode=!spectator]
execute if entity @s[team=runners] unless score 逃者可追踪:敌人 mh.settings matches 1 unless score 逃者可追踪:队友 mh.settings matches 1 run scoreboard players set #can_have_compass mh.temp 0
execute if entity @s[team=runners] unless score 逃者可追踪:敌人 mh.settings matches 1 if score 逃者可追踪:队友 mh.settings matches 1 unless score #runners_count mh.temp matches 2.. run scoreboard players set #can_have_compass mh.temp 0
...
# (优先度应低于背包事件,否则无效)
execute if items entity @s weapon.offhand *[minecraft:custom_data~{"mh:tracker":{}}] run tag @s add mh.offhand.tracker
execute unless items entity @s weapon.offhand *[minecraft:custom_data~{"mh:tracker":{}}] run tag @s remove mh.offhand.tracker
execute if items entity @s player.cursor *[minecraft:custom_data~{"mh:tracker":{}}] run tag @s add mh.cursor.tracker
execute unless items entity @s player.cursor *[minecraft:custom_data~{"mh:tracker":{}}] run tag @s remove mh.cursor.tracker
