...
# 如果没有这名玩家, 则直接报错
$execute unless entity @a[name=$(raw)] run data modify storage mh:temp TextBuffer set value '$(raw)'
...
execute unless data storage gu:main out run tellraw @s [{"selector": "@p[tag=mh.chosen]"},{"text":"不是你可以追踪的目标","color": "red"}]
# 移除所有玩家的标签    
