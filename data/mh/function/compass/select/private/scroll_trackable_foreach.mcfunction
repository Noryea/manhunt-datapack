...
# 如果已经遍历到最后，则第一个人是新目标
execute if score #old_ptr mh.temp matches -1 if score #iter mh.temp = #len mh.temp as @a[sort=arbitrary,tag=mh.trackable,limit=1] at @s run \
    return run function mh:gu/generate

# 如果已经找到旧目标的指针位置,则不往下运行
execute unless score #old_ptr mh.temp matches -1 run \
    return fail
