...
execute if score 追踪器:丢弃时触发 mh.settings matches 1 run function mh:compass/switch_target
...
execute on origin unless items entity @s weapon.mainhand * unless entity @s[tag=mh.cursor.tracker] if entity @s[tag=mh.offhand.tracker] if items entity @s weapon.offhand * store success score #result mh.temp run item replace entity @s weapon.mainhand from entity @n[type=item,distance=..7,tag=mh.item.pick] contents
execute on origin unless items entity @s weapon.mainhand * unless entity @s[tag=mh.cursor.tracker] unless entity @s[tag=mh.offhand.tracker] store success score #result mh.temp run item replace entity @s weapon.mainhand from entity @n[type=item,distance=..7,tag=mh.item.pick] contents
# 如果玩家有mh.cursor.tracker标签, 尝试从掉落物的contents槽复制到玩家指针
execute on origin if entity @s[tag=mh.cursor.tracker] unless items entity @s player.cursor * store success score #result mh.temp run item replace entity @s player.cursor from entity @n[type=item,distance=..7,tag=mh.item.pick] contents
# 如果玩家有mh.offhand.tracker标签, 尝试从掉落物的contents槽复制到玩家副手
execute on origin if entity @s[tag=mh.offhand.tracker] unless items entity @s weapon.offhand * store success score #result mh.temp run item replace entity @s weapon.offhand from entity @n[type=item,distance=..7,tag=mh.item.pick] contents
