...
execute unless score 追踪器:更新模式 mh.settings matches 1..3 run \
  ...
  "  使用冷却=", {"score": { "objective": "mh.settings", "name": "追踪器:右键更新周期游戏刻" }, "color": "aqua", "underlined": true, "hoverEvent":{"action": "show_text","value":"(可修改成0~200,其中0表示无冷却)"},"clickEvent":{"action":"suggest_command","value":"/function mh:setting/menu/set_value/use_delay {Value: \"请输入\"}"}},{"text": "ticks", "color": "aqua"}]
