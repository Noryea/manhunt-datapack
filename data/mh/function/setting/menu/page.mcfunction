...
tellraw @s [{"text":"[清空玩家状态]","color":"yellow","hoverEvent": {"action": "show_text","value":"重置所有玩家的血量、背包、重生点、进度和配方"},"clickEvent": {"action": "suggest_command","value": "/function #mh:recover_all"}}, \
    "",\
    {"text":"[冻结猎人..]","color":"#b3ffff","hoverEvent": {"action": "show_text","value":"/function #mh:set_freeze {Value:<冻结秒数>}"},"clickEvent": {"action": "suggest_command","value": "/function #mh:set_freeze {Value:\"请输入\"}"}}]
