...
execute if score 总是更新指南针 mh.settings matches 1 run tellraw @s [{"text":"总是更新指南针*: ","hoverEvent": {"action": "show_text","value": "说明：当开启时，每次更新将在指南针物品数据写入当前的游戏时间。这可以避免指南针因存储坐标不变而没有更新动画。"}},{"text":"[开启]","color":"green","hoverEvent":{"action": "show_text","value":"点击切换"},"clickEvent":{"action":"run_command","value":"/function mh:settings/menu/click_events/switch/compass_store_gametime"}}]
execute unless score 总是更新指南针 mh.settings matches 1 run tellraw @s [{"text":"总是更新指南针*: ","hoverEvent": {"action": "show_text","value": "说明：当开启时，每次更新将在指南针物品数据写入当前的游戏时间。这可以避免指南针因存储坐标不变而没有更新动画。"}},{"text":"[关闭]","color":"red","hoverEvent":{"action": "show_text","value":"点击切换"},"clickEvent":{"action":"run_command","value":"/function mh:settings/menu/click_events/switch/compass_store_gametime"}}]
