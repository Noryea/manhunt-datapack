...
execute unless score 追踪器:更新模式 mh.settings matches 1..3 run scoreboard players set 追踪器:更新模式 mh.settings 1
...
execute if score 追踪器:更新模式 mh.settings matches 2 run \
    tellraw @s ["指南针更新模式: ",{"text":"[手持更新]","color":"green","hoverEvent":{"action": "show_text","value":["只有玩家手持指南针时才更新","\n\n性能开销: ",{"text":"中","color":"yellow"}]},"clickEvent":{"action":"run_command","value":"/function mh:setting/menu/set_value/switch_refresh_mode"}}, \
...
execute if score 追踪器:更新模式 mh.settings matches 3 run \
    tellraw @s ["指南针更新模式: ",{"text":"[定期更新]","color":"yellow","hoverEvent":{"action": "show_text","value":["定期更新玩家背包中所有指南针","\n\n性能开销: ",{"text":"大","color":"yellow"}]},"clickEvent":{"action":"run_command","value":"/function mh:setting/menu/set_value/switch_refresh_mode"}}, \
