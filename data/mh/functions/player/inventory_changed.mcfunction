...
## 给予指南针 & 检测丢弃
# 检测指南针丢弃
execute if score #can_have_compass mh.temp matches 1 anchored eyes positioned ^ ^ ^ as @e[type=item,limit=1,sort=nearest,distance=..6,tag=!mh.item.pick] if items entity @s contents *[minecraft:custom_data~{"mh:tracker":{}}] run \
