...
execute on origin if score #result mh.temp matches 1.. unless items entity @s player.cursor * unless entity @s[tag=mh.cursor.tracker] if items entity @s weapon.mainhand * run \
    scoreboard players set #flag mh.temp 1
# (进一步判断:物品不是从副手丢出来的)
execute on origin if entity @s[tag=mh.offhand.tracker] unless items entity @s weapon.offhand * run \
    scoreboard players set #flag mh.temp 0
# 杀死并early return
...
execute if score 追踪器:丢弃时触发 mh.settings matches 2 if items entity @s contents compass \
    store success score #result mh.temp run data modify entity @s Item.id set value "minecraft:writable_book"
    
## 玩家可以通过丢弃退出编辑模式
# 变回指南针
execute if score #result mh.temp matches 0 if items entity @s contents writable_book run \
    item modify entity @s contents [ \
        {function:"set_components",components:{"!minecraft:writable_book_content":{}}}, \
...
execute on origin unless items entity @s weapon.mainhand * unless entity @s[tag=mh.cursor.tracker] if entity @s[tag=mh.offhand.tracker] if items entity @s weapon.offhand * \
    store success score #result mh.temp run \
        item replace entity @s weapon.mainhand from entity @e[type=item,limit=1,sort=nearest,distance=..7,tag=mh.item.pick] contents
execute on origin unless items entity @s weapon.mainhand * unless entity @s[tag=mh.cursor.tracker] unless entity @s[tag=mh.offhand.tracker] \
    store success score #result mh.temp run \
        item replace entity @s weapon.mainhand from entity @e[type=item,limit=1,sort=nearest,distance=..7,tag=mh.item.pick] contents
# 如果玩家有mh.cursor.tracker标签, 尝试从掉落物的contents槽复制到玩家指针
execute on origin if entity @s[tag=mh.cursor.tracker] unless items entity @s player.cursor * \
    store success score #result mh.temp run \
        item replace entity @s player.cursor from entity @e[type=item,limit=1,sort=nearest,distance=..7,tag=mh.item.pick] contents
# 如果玩家有mh.offhand.tracker标签, 尝试从掉落物的contents槽复制到玩家副手
execute on origin if entity @s[tag=mh.offhand.tracker] unless items entity @s weapon.offhand * \
    store success score #result mh.temp run \
        item replace entity @s weapon.offhand from entity @e[type=item,limit=1,sort=nearest,distance=..7,tag=mh.item.pick] contents
...
# 防止物品位置离玩家太远
execute on origin run tp @e[type=item,limit=1,sort=nearest,distance=..7,tag=mh.item.pick] @s
# 设置1tick后刷没
