# track
## Select
execute as @a[tag=trackable,tag=!self] if score @s mh.uid = @p[tag=self] mh.tracking run tag @s add tracking
...
## 信息显示
execute if score 逃者:显示维度 mh.settings matches 1 run data modify storage mh:temp Dimension set from entity @p[tag=tracking,team=runners] Dimension
execute if score 队友:显示维度 mh.settings matches 1 run data modify storage mh:temp Dimension set from entity @p[tag=tracking,team=hunters] Dimension
...
execute if score 队友:显示Y坐标 mh.settings matches 1 if entity @p[tag=tracking,team=hunters] unless entity @a[tag=tracking,distance=0..] run data modify storage mh:temp ActionBar append value '[{"text":" Y坐标: ","color":"gray"},{"nbt":"Pos.Y","storage":"mh:temp","color":"red"},"  "]'
execute if score 队友:显示Y坐标 mh.settings matches 2 if entity @p[tag=tracking,team=hunters] run data modify storage mh:temp ActionBar append value '[{"text":" Y坐标: ","color":"gray"},{"nbt":"Pos.Y","storage":"mh:temp","color":"red"},"  "]'
execute if score 队友:显示维度 mh.settings matches 1 if entity @p[tag=tracking,team=hunters] run data modify storage mh:temp ActionBar append value '[{"text":"  维度: ","color":"gray"},{"nbt":"Dimension","storage":"mh:temp","color":"red"},"  "]'
execute if score 队友:显示距离 mh.settings matches 1 if entity @p[tag=tracking,team=hunters] run function mh:compass/distance_math/do
execute if score 队友:显示距离 mh.settings matches 1 if entity @p[tag=tracking,team=hunters] run data modify storage mh:temp ActionBar append value '[{"text":"  距离: ","color":"gray"},{"score":{"name":"distance_小数点前","objective":"mh.temp"},"color":"red"},{"text":"m ","color":"red"}]'
...
# replace
execute if entity @p[tag=tracking] if data entity @s {SelectedItem:{tag:{Tags:["tracker"]}}} in minecraft:overworld run loot replace entity @s weapon.mainhand 1 mine 29999985 254 29999984 minecraft:air{isShulkerMarker:1b}
execute if entity @p[tag=tracking] if data entity @s {Inventory:[{tag:{Tags:["tracker"]},Slot:-106b}]} in minecraft:overworld run loot replace entity @s weapon.offhand 1 mine 29999985 254 29999984 minecraft:air{isShulkerMarker:1b}
