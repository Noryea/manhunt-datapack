...
execute unless entity @a[tag=trackable] run scoreboard players set #拥有指南针数量 mh.temp 1
execute unless entity @a[tag=trackable] run clear @s compass{Tags:["tracker"]}
execute if entity @a[tag=trackable] if data entity @s Inventory[{tag:{Tags:["tracker"]}}] run scoreboard players set #拥有指南针数量 mh.temp 1
execute if entity @a[tag=trackable] if score #拥有指南针数量 mh.temp matches 0 store result score #拥有指南针数量 mh.temp run clear @s compass{Tags:["tracker"]} 0
